...
import { chat, sessions, wallet, astrologer as astrologerApi, API_URL } from '../services/api';
...
const ConsultationRoom = () => {
    ...
    useEffect(() => {
        if (sessionId && myUserId) {
            // Use the centralized API_URL for signaling
            ...
            socket.onmessage = (event) => {
                try {
                    ...
                    } else if (['offer', 'answer', 'candidate', 'hangup'].includes(data.type)) {
                        setIncomingSignal(data);
                        if (data.type === 'hangup') {
                            setMessages(prev => [...prev, { id: Date.now(), text: "Partner ended the call.", sender: 'System', timestamp: new Date().toLocaleTimeString() }]);
                            setTimeout(() => endCall(false), 1500); // false means don't send signal back
                        }
                    } else if (data.type === 'user_disconnected') {
                        setMessages(prev => [...prev, { id: Date.now(), text: "Partner disconnected unexpectedly.", sender: 'System', timestamp: new Date().toLocaleTimeString() }]);
                        // Automatically end call if partner disappears
    ...
    const endCall = async (shouldSendSignal = true) => {
        ...
        try {
            ...
            if (shouldSendSignal && ws.current && ws.current.readyState === WebSocket.OPEN) {
            ...
            await sessions.end(sessionIdRef.current);
