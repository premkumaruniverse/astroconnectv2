...
import AstroShopManager from '../components/AstroShopManager';
import { ShoppingBagIcon as ShopIconOutline } from '@heroicons/react/24/outline';
import { HomeIcon, UserCircleIcon, ChevronRightIcon } from '@heroicons/react/24/outline';
            ...
            <main className="container mx-auto px-4 py-8">
                ...
                <div className="flex space-x-4 mb-8">
                    <button
                        ...
                        className={`flex items-center px-6 py-2 rounded-xl font-bold transition-all ${activeTab === 'overview' ? 'bg-amber-500 text-white shadow-lg' : 'bg-white dark:bg-[#1e293b] text-gray-500 hover:text-amber-500'}`}
                    >
                    ...
                    <button
                        ...
                        className={`flex items-center px-6 py-2 rounded-xl font-bold transition-all ${activeTab === 'shop' ? 'bg-amber-500 text-white shadow-lg' : 'bg-white dark:bg-[#1e293b] text-gray-500 hover:text-amber-500'}`}
                    >
                        <ShopIconOutline className="h-5 w-5 mr-2" />
                        Astro Shop
                    </button>
                </div>
                ...
                {activeTab === 'overview' ? (
                    <>
                        {/* Header Section */}
                        <div className="flex justify-center items-center mb-8 bg-white dark:bg-[#1e293b] p-4 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
                            <div className="flex items-center space-x-4">
                                <span className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Availability</span>
                                <button
                                    onClick={toggleStatus}
                                    className={`relative inline-flex h-8 w-16 items-center rounded-full transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 ${profile.is_online ? 'bg-emerald-500' : 'bg-gray-300 dark:bg-gray-600'}`}
                                >
                                    <span
                                        className={`${profile.is_online ? 'translate-x-9' : 'translate-x-1'
                                            } inline-block h-6 w-6 transform rounded-full bg-white transition-transform duration-300 shadow-sm`}
                                    />
                                </button>
                                <span className={`text-sm font-black uppercase tracking-widest ${profile.is_online ? 'text-emerald-500' : 'text-gray-400'}`}>
